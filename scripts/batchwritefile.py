...
class Game:
    def __init__(self, gameName, platform, gameURL, achievementsWonNoDlc,
        ...
        self.gameNotes = gameNotes.split(';') if gameNotes else None
    ...
    def to_batch_write_item(self):
        def convert(value):
            if isinstance(value, set):
                ...
                return {"L": [{"S" if isinstance(item, str) else "N" if isinstance(item, (int, float)) else "BOOL" if isinstance(item, bool) else "NULL", str(item)} for item in value]}
            else:
                return {"S": str(value)}

        put_request = {
            "PutRequest": {
                "Item": {key: convert(value) for key, value in self.__dict__.items()}
            }
        }
        return put_request
...
csv_file_path = r'C:\Users\curse\OneDrive\Documents\MyGameCollection_ 150923_205603.csv'
...
batch_write_item_list = [{"PutRequest": {"Item": game.to_batch_write_item()["PutRequest"]["Item"]}} for game in games_list]
